...
async function runCustomer(browser) {
  ...
  assert((await page.getByText(/Browse by category/i).count()) > 0, "customer deployed guest home exposes category browsing");
  assert((await page.getByText(/Popular near you/i).count()) > 0, "customer deployed guest home exposes popular stores");
